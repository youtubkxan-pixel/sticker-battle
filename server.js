...
app.get('/', (req, res) => {
  ...
  const f1 = path.join(__dirname, 'index (1).html');
  const f2 = path.join(__dirname, 'index.html');
  if(fs2.existsSync(f1)) return res.sendFile(f1);
  if(fs2.existsSync(f2)) return res.sendFile(f2);
  res.send('Fayl topilmadi');
});
